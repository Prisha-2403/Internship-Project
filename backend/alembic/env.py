...
from app.db.models import Base  # registers every mapper on Base.metadata

config = context.config
# alembic.ini is read by configparser, which treats "%" as interpolation
# syntax. A password containing a percent-encoded character (a "#" becomes
# "%23", for instance) would otherwise blow up before a connection is ever
# attempted, so escape it on the way in.
